...
## First-party bundled plugin packages

First-party plugins can live under `plugins/*` for local development, but Pi Web mode is now built into `@earendil-works/pi-coding-agent`. Bundled plugins must be copied into `dist/web` by the Web mode build instead of being published as independent npm packages.

A separate plugin package should:

- use type-only imports from `@earendil-works/pi-coding-agent/web-plugin-api` when it needs shared Pi Web mode plugin interfaces; this subpath is currently a `.d.ts`-only dogfooding surface, not a runtime JavaScript module;
