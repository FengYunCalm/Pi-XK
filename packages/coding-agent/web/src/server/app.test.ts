...
import { MAX_IMAGE_PREVIEW_BYTES } from "../shared/workspaceFiles.ts";
...
beforeEach(async () => {
	...
	app = await buildApp({
		...
		piWebPlugins: {
			manifest: () =>
				Promise.resolve({
					plugins: [{ id: "fake", module: "/pi-web-plugins/fake/plugin.js?v=1", source: "test", scope: "local" }],
				}),
			readAsset: (pluginId, assetPath) =>
				Promise.resolve(
					pluginId === "fake" && assetPath === "plugin.js"
						? { content: Buffer.from("export default {};"), contentType: "application/javascript; charset=utf-8" }
						: undefined,
				),
		},
		clientDist: false,
...
describe("buildApp", () => {
	...
	it("serves supported workspace images as previews", async () => {
		...
		const previewResponse = await app.inject({
			method: "GET",
			url: `/api/projects/${project.id}/workspaces/${workspace.id}/file/preview?path=${encodeURIComponent("diagram.svg")}`,
		});
		...
		const rejectedResponse = await app.inject({
			method: "GET",
			url: `/api/projects/${project.id}/workspaces/${workspace.id}/file/preview?path=${encodeURIComponent("note.txt")}`,
		});
		...
		const tooLargeResponse = await app.inject({
			method: "GET",
			url: `/api/projects/${project.id}/workspaces/${workspace.id}/file/preview?path=${encodeURIComponent("huge.png")}`,
		});
