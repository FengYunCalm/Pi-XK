import { accessSync, constants, existsSync, type Stats, statSync } from "node:fs";
...
export function checkNodePtyDarwinSpawnHelper(
	options: NodePtyDarwinSpawnHelperCheckOptions = {},
): NodePtyDarwinSpawnHelperCheck {
	...
	try {
		nodePtyPackageJsonPath =
			options.nodePtyPackageJsonPath ?? (options.resolveNodePtyPackageJson ?? resolveNodePtyPackageJson)();
	} catch (error) {
	...
	try {
		...
		return {
			...
			helperPath,
			nodePtyRoot,
			fixCommand: chmodFixCommand(helperPath),
		};
	}
}

export function formatNodePtyDarwinSpawnHelperCheck(
	check: NodePtyDarwinSpawnHelperCheck,
): FormattedNodePtyDarwinSpawnHelperCheck {
...
function failureDetails(
	check: Exclude<NodePtyDarwinSpawnHelperCheck, { status: "ok" | "skipped" | "spawn-helper-not-executable" }>,
): string[] {
