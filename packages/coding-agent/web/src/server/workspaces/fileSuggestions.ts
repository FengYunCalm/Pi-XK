...
import type { ClientFileSuggestion } from "../types.ts";
...
async function listFilesForScope(
	cwd: string,
	scope: FileSuggestionScope | undefined,
	exec: NonNullable<FileSuggestionDependencies["execFile"]>,
): Promise<ClientFileSuggestion[]> {
...
async function listGitFiles(
	...
	return [...withDirectories(lines(tracked), "tracked"), ...withDirectories(lines(untracked), "untracked")];
}

async function listPlainFiles(
	cwd: string,
	exec: NonNullable<FileSuggestionDependencies["execFile"]>,
