import { isSessionActive, isWorkspaceActivityActive } from "../../shared/activity.ts";
import type {
	RealtimeEvent,
	SessionActivity,
	SessionStatus,
...
export class WorkspaceActivityService {
	...
	private readonly publisher: WorkspaceActivityPublisher | undefined;

	constructor(publisher?: WorkspaceActivityPublisher) {
	...
	snapshot(): WorkspaceActivityResponse {
		return {
			workspaces: this.activeCwds()
				.map((cwd) => this.summaryForCwd(cwd))
	...
	private summaryForCwd(cwd: string): WorkspaceActivity {
		return {
			cwd,
			hasSessionActivity: [...this.sessions.values()].some(
				(record) => record.cwd === cwd && isSessionActive(record.status, record.activity),
			),
