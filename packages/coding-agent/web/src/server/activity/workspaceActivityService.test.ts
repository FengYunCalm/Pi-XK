...
describe("WorkspaceActivityService", () => {
	it("publishes and snapshots session activity by cwd", () => {
		...
		expect(events.at(-1)).toMatchObject({
			...
			activity: { cwd: "/repo", hasSessionActivity: true, hasTerminalActivity: false },
		});
		...
		expect(events.at(-1)).toMatchObject({
			type: "workspace.activity",
			activity: { cwd: "/repo", hasSessionActivity: false, hasTerminalActivity: false },
		});
	});
	...
	it("clears stale active activity when an idle status arrives", () => {
		...
		service.applySessionActivity("/repo", {
			sessionId: "s1",
			phase: "active",
			label: "running tool",
			detail: "read",
	...
	it("combines sessions and terminals and clears closed terminals", () => {
		...
		expect(service.snapshot().workspaces).toMatchObject([
			{ cwd: "/repo", hasSessionActivity: true, hasTerminalActivity: true },
		]);
