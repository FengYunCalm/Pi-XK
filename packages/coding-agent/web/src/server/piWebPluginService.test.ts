...
import { type PiPackageProvider, PiWebPluginService } from "./piWebPluginService.ts";
...
describe("PiWebPluginService", () => {
	it("discovers local plugins and serves assets", async () => {
		...
		await writePlugin(pluginDir, {
			...
			files: {
				"pi-web-plugin.js":
					"export default { apiVersion: 1, name: 'Info', activate: () => ({ contributions: {} }) };",
			},
		});
	...
	it("discovers Pi package plugins through an injected package provider", async () => {
		...
		await writePlugin(packageDir, {
			...
			files: {
				"dist/review.js":
					"export default { apiVersion: 1, name: 'Review', activate: () => ({ contributions: {} }) };",
			},
		});
	...
	it("discovers source checkout plugin packages without symlinks", async () => {
		...
		await writePlugin(join(tempDir, "plugins", "source-dev"), {
			...
			files: {
				"dist/pi-web-plugin.js":
					"export default { apiVersion: 1, name: 'Source Dev', activate: () => ({ contributions: {} }) };",
			},
		});
		...
		expect(manifest.plugins).toEqual(
			expect.arrayContaining([expect.objectContaining({ id: "source-dev", source: "dev", scope: "local" })]),
		);
	...
	it("discovers local plugins through symlinks for development", async () => {
		...
		await writePlugin(pluginDir, {
			...
			files: {
				"pi-web-plugin.js":
					"export default { apiVersion: 1, name: 'Dev', activate: () => ({ contributions: {} }) };",
			},
		});
	...
	it("skips legacy metadata shortcuts and unsafe module paths", async () => {
		...
		await expect(
			new PiWebPluginService({
				roots: [{ path: legacyRoot, source: "test", scope: "local" }],
				packageProvider: false,
			}).manifest(),
		).resolves.toEqual({ plugins: [] });
		await expect(
			new PiWebPluginService({
				roots: [{ path: unsafeRoot, source: "test", scope: "local" }],
				packageProvider: false,
			}).manifest(),
		).resolves.toEqual({ plugins: [] });
	});
...
async function writePlugin(
	root: string,
	options: { packageJson: unknown; files: Record<string, string> },
): Promise<void> {
