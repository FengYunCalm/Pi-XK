...
import {
	type AuthProviderModelRegistry,
	getLoginProviderOptions,
	getLogoutProviderOptions,
	isApiKeyLoginProvider,
} from "./authProviderOptions";
...
function registry(): AuthProviderModelRegistry {
	...
	return {
		...
		getProviderDisplayName: (provider: string) =>
			({ anthropic: "Anthropic", openai: "OpenAI", custom: "Custom" })[provider] ?? provider,
		getProviderAuthStatus: (provider: string) =>
			provider === "openai" ? { configured: true, source: "stored" } : { configured: false },
	};
}
...
describe("auth provider options", () => {
	...
	it("includes Anthropic in both OAuth and API key login options", () => {
		...
		expect(options).not.toEqual(
			expect.arrayContaining([expect.objectContaining({ id: "openai-codex", authType: "api_key" })]),
		);
	});
