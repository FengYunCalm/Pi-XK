...
import type { AuthProviderOption, AuthProviderStatus, AuthType } from "../../shared/apiTypes.ts";
...
export function getLoginProviderOptions(
	modelRegistry: AuthProviderModelRegistry,
	authType?: AuthType,
): AuthProviderOption[] {
...
function filterAndSort(options: AuthProviderOption[], authType?: AuthType): AuthProviderOption[] {
	...
	return filtered.sort(
		(a, b) => a.name.localeCompare(b.name) || a.authType.localeCompare(b.authType) || a.id.localeCompare(b.id),
	);
}
