...
import type { AuthProvidersResponse, AuthType, OAuthFlowState } from "../../shared/apiTypes.ts";
import { getLoginProviderOptions, getLogoutProviderOptions } from "./authProviderOptions.ts";
import { OAuthLoginFlowService } from "./oauthLoginFlowService.ts";
...
export class AuthService {
	...
	authProviders(mode: "login" | "logout", authType?: AuthType): AuthProvidersResponse {
		...
		const providers =
			mode === "logout"
				? getLogoutProviderOptions(this.modelRegistry)
				: getLoginProviderOptions(this.modelRegistry, authType);
		return { providers };
	}
