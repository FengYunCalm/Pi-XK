...
import type { CommandOption, OAuthFlowState } from "../../shared/apiTypes.ts";
...
export class OAuthLoginFlowService {
	...
	start(options: {
		...
		const callbacks: OAuthLoginCallbacks = {
			signal: abort.signal,
				...
				onDeviceCode: (info) => {
					...
					this.updateState(record, {
						...record.state,
						auth: { url: info.verificationUri, instructions: `Enter code ${info.userCode}` },
						progress: [...record.state.progress, `Device code: ${info.userCode}`],
					});
				},
			...
			onManualCodeInput: () =>
				this.waitForPrompt(
					record,
					{ message: "Paste the callback URL or authorization code", allowEmpty: false },
					"manual",
				),
		...
		void options.authStorage
			.login(options.providerId, callbacks)
			.then(() => {
				if (!this.isCurrentRunning(record)) return;
				record.pending = undefined;
				this.markTerminal(record, {
					...withoutInteraction(record.state),
					status: "complete",
					progress: [...record.state.progress, "Login complete"],
				});
				options.onComplete?.();
			})
			.catch((error: unknown) => {
	...
	private scheduleRunningExpiry(record: OAuthFlowRecord): void {
		...
		this.setTimer(record, this.runningTtlMs, () => {
			this.expireRunningFlow(record);
		});
	}
	...
	private expireRunningFlow(record: OAuthFlowRecord): void {
		...
		this.markTerminal(record, {
			...withoutInteraction(record.state),
			status: "error",
			error: "OAuth login flow expired",
		});
...
function cloneState(state: OAuthFlowState): OAuthFlowState {
	return {
		...
		...(state.select === undefined
			? {}
			: { select: { ...state.select, options: state.select.options.map((option) => ({ ...option })) } }),
	};
}
