...
import type { GlobalSessionEvent, SessionUiEvent } from "../../shared/apiTypes.ts";
...
describe("PiSessionService", () => {
	it("starts sessions through an injected runtime creator", async () => {
		...
		expect(
			hub.globalEvents.some((event) => event.type === "status.update" && event.status.sessionId === "session-1"),
		).toBe(true);
	...
	it("clears stale active activity once a previously active session becomes idle", async () => {
		...
		try {
			...
			const activityPhases = () =>
				hub.globalEvents.filter((event) => event.type === "activity.update").map((event) => event.activity.phase);
	...
	it("lists only shared CLI sessions from the session-manager gateway", async () => {
			...
			sessionManager: {
				...
				list: () =>
					Promise.resolve([
						...
						{ ...sessionRecord("second"), messageCount: 2, firstMessage: "bye", allMessagesText: "bye" },
					]),
		...
		expect(sessions[1]).toMatchObject({ id: "second" });

		await service.dispose();
	});

	it("reconciles workspace activity when no shared CLI sessions are listed", async () => {
		...
		const service = new PiSessionService(new CapturingSessionEventHub(), {
			...
			workspaceActivity: {
				...
				reconcileSessionActivity: (cwd, sessionIds) => {
					reconciliations.push({ cwd, sessionIds: [...sessionIds] });
				},
			},
			heartbeatIntervalMs: 60_000,
		});
	...
	it("does not append queued prompts to the transcript before delivery", async () => {
		...
		expect(fake.calls.prompt).toEqual([
			{ text: "Wait for the current turn", options: { streamingBehavior: "followUp" } },
		]);
	...
	it("holds prompts sent during compaction until compaction finishes", async () => {
		...
		await expect(service.status("compacting-session")).resolves.toMatchObject({
			pendingMessageCount: 2,
			queuedMessages: [
				{ kind: "followUp", text: "Start task 1" },
				{ kind: "followUp", text: "Then task 2" },
			],
		});
		...
		expect(
			hub.sessionEvents.some(
				({ event }) => event.type === "message.append" && JSON.stringify(event.message).includes("Start task 1"),
			),
		).toBe(true);
		await expect(service.status("compacting-session")).resolves.toMatchObject({
			pendingMessageCount: 1,
	...
	it("clears prompts queued during compaction when aborting active work", async () => {
		...
		expect(fake.calls.clearQueue).toBe(1);
		expect(fake.calls.prompt).toEqual([]);
		await expect(service.status("abort-compaction-session")).resolves.toMatchObject({
			pendingMessageCount: 0,
			queuedMessages: [],
		});
		await service.dispose();
	...
	it("refreshes auth state and dedupes warnings when logout removes the current model's credentials", async () => {
		...
		const warningCount = () =>
			hub.sessionEvents.filter(
				({ event }) =>
					event.type === "command.output" &&
					event.level === "error" &&
					event.message.includes("anthropic/claude-3-5-sonnet-20241022"),
			).length;
		expect(warningCount()).toBe(1);
		expect(
			hub.globalEvents.some((event) => event.type === "status.update" && event.status.sessionId === "auth-session"),
		).toBe(true);
