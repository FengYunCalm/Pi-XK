...
import { type CommandActiveSession, type CommandSession, SessionCommandService } from "./sessionCommandService.ts";
...
describe("SessionCommandService", () => {
	it("rejects unknown commands and forwards runtime commands as prompts", async () => {
		...
		await expect(service.run("s1", "/missing")).resolves.toEqual({
			type: "unsupported",
	...
	it("starts compaction and publishes completion", async () => {
		...
		await expect(service.run("s1", "/compact focus on tests")).resolves.toEqual({
			type: "done",
	...
	it("creates fork selection requests from newest message to oldest and responds with selected entry", async () => {
		...
		expect(result).toMatchObject({
			...
			options: [{ value: "newest" }, { value: "middle" }, { value: "oldest" }],
		});
		...
		await expect(service.respond("s1", result.requestId, "newest")).resolves.toMatchObject({
			...
			session: { id: "s1" },
			promptDraft: "newest message",
		});
		expect(active.runtime.fork).toHaveBeenCalledWith("newest");
		await expect(service.respond("s1", result.requestId, "newest")).resolves.toEqual({
			type: "unsupported",
			message: "Command request expired",
		});
	});
	...
	it("names forked sessions from the source title with the next available counter", async () => {
		...
		expect(events.publish).toHaveBeenCalledWith("forked", {
			type: "session.name",
			sessionId: "forked",
			name: "Build auth — Fork 2",
		});
	});
