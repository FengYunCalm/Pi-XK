...
import type { SessionUiEvent } from "../../shared/apiTypes.ts";
import type { ClientCommandResult, ClientSession } from "../types.ts";
import { isBuiltinCommand } from "./builtinCommands.ts";
...
export type GetCommandActiveSession<TSession extends CommandSession = CommandSession> = (
	sessionId: string,
) => Promise<CommandActiveSession<TSession>>;
...
export class SessionCommandService<TSession extends CommandSession = CommandSession> {
	...
	constructor(
		getActive: GetCommandActiveSession<TSession>,
	...
	private fork(active: CommandActiveSession<TSession>): ClientCommandResult {
		...
		return {
			...
			options: [...messages]
				.reverse()
				.map((message) => ({ value: message.entryId, label: truncate(message.text, 140) })),
		};
	}
	...
	private isRuntimeCommand(session: TSession, name: string): boolean {
		return (
			session.extensionRunner.getRegisteredCommands().some((command) => command.invocationName === name) ||
...
): Partial<Pick<Extract<ClientCommandResult, { type: "done" }>, "promptDraft">> {
...
function formatCompactionResult(result: { summary: string; tokensBefore: number }): string {
	return ["Compaction complete.", `Tokens before: ${String(result.tokensBefore)}`, "", result.summary].join("\n");
}
