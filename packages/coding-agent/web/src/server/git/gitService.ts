...
import type { GitDiffResponse, GitFileState, GitStatusFile, GitStatusResponse } from "../../shared/apiTypes.ts";
import { normalizeRelativePath } from "../workspaces/pathSafety.ts";
...
async function runGit(
	...
	return new Promise((resolve, reject) => {
		...
		child.on("close", (code) => {
			clearTimeout(timer);
			resolve({ code: code ?? 1, stdout: stdout.toString("utf8"), stderr: stderr.toString("utf8"), truncated });
		});
	});
}
