...
import type { PiWebComponentStatus } from "../shared/apiTypes.ts";
import { comparePackageVersions, getPiWebStatus, getPiWebVersionStatus } from "./piWebStatus.ts";
...
describe("PI WEB status", () => {
	...
	it("reports stale session daemon versions as messages", async () => {
		...
		const daemon = daemonWithComponent({
			...
			stale: true,
			available: true,
			installation: { kind: "pi-package", source: "npm:pi", scope: "user", path: "/tmp/pi-web" },
		});
		...
		expect(status.components.sessiond.installation).toMatchObject({
			kind: "pi-package",
			source: "npm:pi",
			scope: "user",
		});
