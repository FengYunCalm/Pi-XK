...
import { readWorkspaceFile } from "./workspaces/fileContentService.ts";
import { listWorkspaceTree } from "./workspaces/fileTreeService.ts";
import { readWorkspaceImagePreview } from "./workspaces/imagePreviewService.ts";
...
export function registerWorkspaceExplorerRoutes(
	...
	app.get<{ Params: { projectId: string; workspaceId: string }; Querystring: { path?: string } }>(
		"/api/projects/:projectId/workspaces/:workspaceId/tree",
		async (request, reply) => {
			try {
	...
	app.get<{ Params: { projectId: string; workspaceId: string }; Querystring: { path?: string } }>(
		"/api/projects/:projectId/workspaces/:workspaceId/file",
		async (request, reply) => {
			try {
				const context = await resolveWorkspaceContext(
					projects,
	...
	app.get<{ Params: { projectId: string; workspaceId: string }; Querystring: { path?: string } }>(
		"/api/projects/:projectId/workspaces/:workspaceId/file/preview",
		async (request, reply) => {
			try {
				const context = await resolveWorkspaceContext(
					projects,
