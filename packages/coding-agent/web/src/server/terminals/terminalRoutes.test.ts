...
import type { TerminalCommandRun, TerminalCommandRunFilter } from "../../shared/apiTypes.ts";
import { registerTerminalRoutes, type TerminalRouteService } from "./terminalRoutes.ts";
...
describe("terminal routes", () => {
	it("applies the initial socket size before attaching and replaying output", async () => {
		...
		await expect(nextMessage(socket)).resolves.toBe(
			JSON.stringify({ type: "output", data: "replayed", replay: true }),
		);
	...
	it("creates and lists terminal command runs with filters", async () => {
		...
		expect(createResponse.json<TerminalCommandRun>()).toMatchObject({
			id: "run1",
			terminalId: "t-run",
			status: "running",
		});
		...
		const listResponse = await app.inject({
			method: "GET",
			url: `/terminal-command-runs?projectId=p1&statuses=running&metadata=${encodeURIComponent(JSON.stringify({ "pi.operation": "test" }))}`,
		});

		expect(listResponse.statusCode).toBe(200);
		expect(listResponse.json<TerminalCommandRun[]>()).toHaveLength(1);
		expect(terminals.filters).toEqual([
			{ projectId: "p1", statuses: ["running"], metadata: { "pi.operation": "test" } },
		]);
