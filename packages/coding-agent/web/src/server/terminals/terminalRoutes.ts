...
import type { TerminalCommandRun, TerminalCommandRunFilter, TerminalCommandRunStatus } from "../../shared/apiTypes.ts";
import type { RunTerminalCommandOptions, TerminalInfo } from "./terminalService.ts";
import { parseTerminalSize } from "./terminalSize.ts";
...
export function registerTerminalRoutes(app: FastifyInstance, terminals: TerminalRouteService, prefix = ""): void {
	...
	app.post<{ Body: { cwd: string; name?: string; cols?: number; rows?: number } }>(
		`${prefix}/terminals`,
		(request, reply) => {
			try {
	...
	app.get<{ Params: { terminalId: string }; Querystring: { cols?: string; rows?: string } }>(
...
type ClientTerminalMessage = { type: "input"; data: string } | { type: "resize"; cols: number; rows: number };
...
function parseCommandRunFilter(query: TerminalCommandRunQuery): TerminalCommandRunFilter {
	const metadata =
		query.metadata === undefined || query.metadata === "" ? undefined : parseMetadataFilter(query.metadata);
	const statuses =
...
function parseMetadataFilter(value: string): Record<string, string> {
	...
	return Object.fromEntries(
		Object.entries(parsed).map(([key, metadataValue]) => {
