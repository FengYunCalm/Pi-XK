...
export class PiWebPluginService {
	...
	constructor(options: PiWebPluginServiceOptions = {}) {
		...
		this.packageProvider =
			options.packageProvider === false
				? undefined
				: (options.packageProvider ?? new DefaultPiPackageProvider(cwd, agentDir));
	}
	...
	private async discoverPiPackagePlugins(packageProvider: PiPackageProvider): Promise<PluginRecord[]> {
		...
		for (const configuredPackage of packageProvider.listPackages()) {
			const root =
				...
				packageProvider.getInstalledPath(configuredPackage.source, configuredPackage.scope);
...
function defaultPluginRoots(cwd: string): LocalPluginRoot[] {
	...
	const packageRoot = runtimeRootPath(moduleDir);
	return [
...
function runtimeRootPath(moduleDir: string): string {
	const embeddedRoot = join(moduleDir, "..");
	if (existsSync(join(embeddedRoot, "package.json")) || existsSync(join(embeddedRoot, "pi-web-plugins"))) {
		return embeddedRoot;
	}
...
function bundledPluginRoot(packageRoot: string): string {
	const embeddedPlugins = join(packageRoot, "pi-web-plugins");
	if (existsSync(embeddedPlugins)) return embeddedPlugins;
...
async function discoverLocalRoot(root: LocalPluginRoot): Promise<PluginRecord[]> {
	...
	for (const entry of entries) {
		...
		const pluginStat = entry.isDirectory()
			? undefined
			: entry.isSymbolicLink()
				? await stat(pluginRoot).catch(() => undefined)
				: undefined;
...
async function discoverPluginEntries(root: string, config: PiWebPackageConfig): Promise<ArraylessPluginRecord[]> {
	...
	for (const entry of config.plugins) {
		if (!isSafeRelativePath(entry.module))
			throw new Error(`Unsafe PI WEB plugin module path for ${entry.id}: ${entry.module}`);
		const entryPath = join(root, entry.module);
		const entryStat = await stat(entryPath).catch(() => undefined);
		if (entryStat?.isFile() !== true)
			throw new Error(`PI WEB plugin module not found for ${entry.id}: ${entry.module}`);
...
function parsePluginEntries(piWeb: Record<string, unknown>, packagePath: string): PiWebPluginEntry[] {
	if (piWeb["plugin"] !== undefined)
		throw new Error(
			`Unsupported PI WEB plugin metadata in ${packagePath}: use piWeb.plugins with { id, module } entries`,
		);
	...
	return plugins.map((entry, index): PiWebPluginEntry => {
		...
		if (typeof module !== "string" || module === "")
			throw new Error(`Invalid PI WEB plugin module for ${id} in ${packagePath}`);
		return { id, module };
	});
}
