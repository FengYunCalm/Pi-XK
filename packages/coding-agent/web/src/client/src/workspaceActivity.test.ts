...
describe("workspace activity aggregation", () => {
	...
	it("uses a terminal indicator only when there is no session activity", () => {
		expect(
			workspaceActivityIndicator(activity("/repo", { hasSessionActivity: false, hasTerminalActivity: true })),
		).toBe("terminal");
		expect(
			workspaceActivityIndicator(activity("/repo", { hasSessionActivity: true, hasTerminalActivity: true })),
		).toBe("session");
		expect(
			workspaceActivityIndicator(activity("/repo", { hasSessionActivity: false, hasTerminalActivity: false })),
		).toBeUndefined();
	});
	...
	it("returns a project terminal indicator only when matched workspaces have no session activity", () => {
		expect(
			projectActivityIndicator(project("p1", "/repo"), [], {
				"/repo": activity("/repo", { hasSessionActivity: false, hasTerminalActivity: true }),
			}),
		).toBe("terminal");
		expect(
			projectActivityIndicator(project("p1", "/repo"), [], {
				"/repo": activity("/repo", { hasSessionActivity: true, hasTerminalActivity: true }),
			}),
		).toBe("session");
	});
	...
	it("falls back to project path matching before workspaces have been loaded", () => {
		expect(
			projectActivityIndicator(project("p1", "/repo"), [], { "/repo/packages/app": activity("/repo/packages/app") }),
		).toBe("session");
