...
describe("API parsers", () => {
	...
	it("validates file content responses", () => {
		...
		).toMatchObject({ path: "logo.png", mediaType: "image", mimeType: "image/png" });
	...
	it("parses command result variants", () => {
		expect(parseCommandResult({ type: "unsupported", message: "nope" })).toEqual({
			type: "unsupported",
			message: "nope",
		});
		expect(
		...
		expect(parseCommandResult({ type: "done", message: "ok", promptDraft: "resend me" })).toEqual({
			type: "done",
			message: "ok",
