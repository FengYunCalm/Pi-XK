...
import {
	...
	type ChatScrollElement,
	type ChatScrollScheduler,
	type ChatScrollStorage,
...
describe("ChatScrollController", () => {
	...
	it("saves and restores the visible anchor nearest the viewport top", () => {
		...
		expect(JSON.parse(storage.getItem(chatScrollStorageKey("s1")) ?? "{}")).toEqual({
			mode: "anchor",
			anchorId: "e:6",
			offset: -20,
		});
	...
	it("reports missing stored anchors instead of forcing bottom when requested", () => {
		...
		expect(
			controller.restorePosition("s1", scroller, [new FakeArticle(10, 40, "m:9")], { fallbackToBottom: false }),
		).toEqual({ status: "missing", position });
	...
	it("saves explicit bottom mode when the user is at the bottom", () => {
		...
		expect(
			controller.savePosition("s1", new FakeScroller(699, 1000, 300, 0, 300), [new FakeArticle(0, 30, "m:0")]),
		).toBe("saved");
	...
	it("captures the session id when scheduling a delayed save", () => {
		...
		controller.scheduleSave("s1", (sessionId) => {
			saved.push(sessionId);
		});
		controller.scheduleSave("s2", (sessionId) => {
			saved.push(sessionId);
		});
		scheduler.runAll();
...
describe("chat scroll helpers", () => {
	...
	it("captures an anchor-relative scroll position", () => {
		expect(captureScrollPosition(new FakeScroller(0, 1000, 100, 100, 200), new FakeArticle(140, 180, "m:3"))).toEqual(
			{ mode: "anchor", anchorId: "m:3", offset: 40 },
		);
	});
});
