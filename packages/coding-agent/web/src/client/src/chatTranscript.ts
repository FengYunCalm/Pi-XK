...
export function applyTranscriptEvent(messages: ChatLine[], event: SessionUiEvent): ChatLine[] | undefined {
	...
	if (event.type === "tool.update")
		return updateToolExecution(messages, event.toolCallId, (part) => mergeToolExecutionUpdate(part, event));
	if (event.type === "tool.end")
		return finalizeToolExecution(
			messages,
			event.toolCallId,
			event.toolName,
			summarizeArgs(event.content),
			event.text,
			event.isError,
			event.content,
			event.details,
	...
	if (event.type === "command.output")
		return [...messages, textMessage(event.level === "error" ? "system" : "tool", event.message)];
	if (event.type === "session.error") return [...messages, textMessage("system", event.message)];
	if (event.type === "message.end")
		return event.message === undefined ? undefined : applyFinalMessage(messages, event.message);
	return undefined;
}
...
function applyFinalMessage(messages: ChatLine[], rawMessage: unknown): ChatLine[] | undefined {
	...
	if (rawToolResult !== undefined) {
		return finalizeToolExecution(
			messages,
			rawToolResult.toolCallId,
			rawToolResult.toolName,
			summarizeArgs(rawToolResult.content),
			rawToolResult.text,
			rawToolResult.isError,
			rawToolResult.content,
			rawToolResult.details,
		);
	}
	...
	if (skillReadIndex >= 0)
		return [...messages.slice(0, skillReadIndex), displayEnded, ...messages.slice(skillReadIndex + 1)];
	const last = messages.at(-1);
	if (last?.role !== displayEnded.role) return [...messages, displayEnded];
	if (displayEnded.role === "assistant" || sameMessageText(last, displayEnded))
		return [...messages.slice(0, -1), displayEnded];
...
function appendToolExecutionStart(
	messages: ChatLine[],
	event: Extract<SessionUiEvent, { type: "tool.start" }>,
): ChatLine[] {
	const skillRead = event.toolName === "read" ? parseSkillReadPath(getString(event.args, "path")) : undefined;
	if (skillRead !== undefined)
		return appendLine(messages, { role: "skill", parts: [{ type: "skillRead", ...skillRead }] });
...
function mergeToolExecutionUpdate(
	...
	event: Extract<SessionUiEvent, { type: "tool.update" }>,
): ToolExecutionPart {
...
function updateToolExecution(
	...
	for (let lineIndex = messages.length - 1; lineIndex >= 0; lineIndex--) {
		...
		const nextLine = {
			...line,
			parts: [...line.parts.slice(0, partIndex), update(part), ...line.parts.slice(partIndex + 1)],
		};
...
function toolResultFromRawMessage(
	message: unknown,
):
	| { toolCallId?: string; toolName: string; text: string; isError: boolean; content: unknown; details: unknown }
	| undefined {
...
function appendLine(messages: ChatLine[], line: ChatLine): ChatLine[] {
	...
	if (last?.role === line.role && line.role !== "skill")
		return [...messages.slice(0, -1), { ...last, parts: [...last.parts, ...line.parts] }];
