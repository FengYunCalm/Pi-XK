...
export class PluginRegistry {
	...
	register(registration: PiWebPluginRegistration): void {
		...
		for (const panel of contributions.workspacePanels ?? [])
			this.workspacePanels.push(this.qualifyWorkspacePanel(id, panel));
		for (const contribution of contributions.workspaceLabels ?? [])
			this.workspaceLabels.push(this.qualifyWorkspaceLabelContribution(id, contribution));
	...
	getWorkspacePanels(): QualifiedWorkspacePanelContribution[] {
		return [...this.workspacePanels].sort(
	...
	private qualifyWorkspacePanel(
		...
		return {
			...
			...(badge === undefined
				? {}
				: { badge: (context: WorkspacePanelContext) => badge(workspacePanelContextFor(context, pluginId)) }),
