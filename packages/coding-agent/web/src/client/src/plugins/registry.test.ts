...
describe("PluginRegistry", () => {
	it("namespaces contribution ids with the owning plugin id", () => {
		...
		expect(registry.getActions(createContext().context).some((action) => action.id === "core:actions.show")).toBe(
			true,
		);
	...
	it("evaluates core action enablement against runtime state", () => {
		...
		const deletable = registry.getActions(
			createContext({ selectedWorkspace: testWorkspace({ isMain: false, isGitWorktree: true }) }).context,
		);
	...
	it("offers delete only for browser-cached new sessions", () => {
		const registry = new PluginRegistry();
		registry.register({ id: "core", plugin: corePlugin });
		...
		const cachedActions = registry.getActions(
			createContext({ selectedSession: markCachedNewSessionInfo(testSession()) }).context,
		);
	...
	it("collects theme contributions in contribution order", () => {
			...
			plugin: {
				...
				activate: () => ({
					contributions: {
						...
						themePairs: [{ id: "pair", name: "Pair", light: "first", dark: "last" }],
					},
				}),
			},
		});
		...
		expect(
			registry
				.getThemes()
				.map((theme) => ({ id: theme.id, pluginId: theme.pluginId, localId: theme.localId, name: theme.name })),
		).toEqual([
					...
					contributions: {
						workspaceLabels: [
							...
							{ id: "hidden", order: 5, visible: () => false, items: () => [{ type: "text", text: "hidden" }] },
							{
								id: "first",
								order: 10,
								items: () => [{ type: "link", text: "web", href: "http://localhost:5173" }],
							},
						],
					},
				}),
