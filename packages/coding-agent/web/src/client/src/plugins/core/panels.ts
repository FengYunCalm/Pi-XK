...
import type { WorkspacePanelContext, WorkspacePanelContribution } from "../types";
...
function renderFileViewer(context: WorkspacePanelContext): TemplateResult {
	...
	if (context.selectedFilePath === undefined || context.selectedFilePath === "")
		return html`<p class="muted">Select a file.</p>`;
...
function renderImageViewer(context: WorkspacePanelContext, file: FileContentResponse): TemplateResult {
	...
	const src = workspaceImagePreviewUrl(context.workspace.projectId, context.workspace.id, file.path, {
...
function renderGit(context: WorkspacePanelContext): TemplateResult {
    ...
    <section class="split">
      <div class="list">
        ${
				status === undefined
					? html`<p class="muted">No status loaded.</p>`
					: !status.isGitRepo
						? html`<p class="muted">Not a git repository.</p>`
						: html`
