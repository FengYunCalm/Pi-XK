...
export function appendShellChunk(messages: ChatLine[], chunk: string): ChatLine[] {
	...
	return [
		...messages.slice(0, -1),
		{ ...last, parts: [...last.parts.slice(0, -1), { ...lastPart, text: lastPart.text + separator + chunk }] },
	];
}

export function finalizeShellMessage(
	messages: ChatLine[],
	event: Extract<SessionUiEvent, { type: "shell.end" }>,
): ChatLine[] {
	const last = messages.at(-1);
	const lastPart = last?.parts.at(-1);
	...
	if (event.fullOutputPath !== undefined && event.fullOutputPath !== "")
		notes.push(`full output: ${event.fullOutputPath}`);
	if (notes.length === 0) return messages;
	return [
		...messages.slice(0, -1),
		{
			...last,
			parts: [...last.parts.slice(0, -1), { ...lastPart, text: `${lastPart.text}\n\n${notes.join("\n")}` }],
		},
	];
}
