...
describe("applyTranscriptEvent", () => {
	...
	it("replaces streamed skill reads when the finalized assistant tool call arrives after the tool result", () => {
		...
		expect(
			...
			{
				role: "skill",
				parts: [{ type: "skillRead", name: "playwright", path: "/skills/playwright/SKILL.md" }],
	...
	it("replaces streamed skill reads when the finalized assistant message includes thinking", () => {
		...
		expect(
		...
		).toEqual([
			{
				role: "assistant",
				parts: [
					{ type: "thinking", text: "load skill" },
					{ type: "skillRead", name: "playwright", path: "/skills/playwright/SKILL.md" },
				],
				meta: { timestamp: "2026-05-09T12:00:00.000Z" },
			},
			{ role: "tool", parts: [{ type: "toolResult", toolName: "read", text: "skill content", isError: false }] },
	...
	it("replaces streamed skill reads when finalized paths differ but the skill name matches", () => {
		...
		expect(
					...
					content: [
						{
							type: "toolCall",
							name: "read",
							arguments: { path: "/home/user/.agents/skills/playwright/SKILL.md" },
						},
					],
					timestamp: "2026-05-09T12:00:00.000Z",
				},
			...
			{
				role: "skill",
				parts: [{ type: "skillRead", name: "playwright", path: "/home/user/.agents/skills/playwright/SKILL.md" }],
				meta: { timestamp: "2026-05-09T12:00:00.000Z" },
			},
			{ role: "tool", parts: [{ type: "toolResult", toolName: "read", text: "skill content", isError: false }] },
		]);
	...
	it("keeps edit tool preview and result updates on one execution card", () => {
		...
		messages =
			applyTranscriptEvent(messages, {
				...
				toolName: "edit",
				toolCallId: "edit-1",
				text: "Edit preview computed.",
				details: { preview: { diff: "-1 old\n+1 new", firstChangedLine: 1 } },
			}) ?? messages;
		messages =
			applyTranscriptEvent(messages, {
				type: "tool.end",
	...
	it("does not merge consecutive streamed skill reads", () => {
		...
		messages =
			applyTranscriptEvent(messages, {
				...
				toolCallId: "1",
				summary: "",
				args: { path: "/skills/playwright/SKILL.md" },
			}) ?? messages;
		messages =
			applyTranscriptEvent(messages, {
				type: "tool.start",
				...
				toolCallId: "2",
				summary: "",
				args: { path: "/skills/sentry-cli/SKILL.md" },
			}) ?? messages;
	...
	it("does not merge different finalized user messages", () => {
		...
		expect(
			applyTranscriptEvent(messages, {
				type: "message.end",
				message: { role: "user", content: "second queued prompt" },
			}),
		).toEqual([textMessage("user", "first queued prompt"), textMessage("user", "second queued prompt")]);
	});
	...
	it("does not merge optimistic user messages after an aborted turn", () => {
		...
		expect(
			applyTranscriptEvent(messages, { type: "message.append", message: { role: "user", content: "new prompt" } }),
		).toEqual([textMessage("user", "stopped prompt"), textMessage("user", "new prompt")]);
	});
	...
	it("replaces a new optimistic user message instead of duplicating it after an aborted turn", () => {
		...
		expect(
			applyTranscriptEvent(messages, {
				type: "message.end",
				message: { role: "user", content: "new prompt", timestamp: "2026-05-09T12:00:00.000Z" },
			}),
		).toEqual([
	...
	it("replaces an optimistic user message when the finalized text matches", () => {
		...
		expect(
			applyTranscriptEvent(messages, {
				type: "message.end",
				message: { role: "user", content: "sent prompt", timestamp: "2026-05-09T12:00:00.000Z" },
			}),
		).toEqual([{ ...textMessage("user", "sent prompt"), meta: { timestamp: "2026-05-09T12:00:00.000Z" } }]);
	});
});
