...
export class KeyboardShortcutDispatcher {
	...
	handle(event: ShortcutKeyEvent, actions: AppAction[]): boolean {
		...
		const sequence =
			this.pendingTokens.length > 0 && !isModifiedShortcut(token) ? [...this.pendingTokens, token] : [token];
...
function normalizeKey(key: string): string | undefined {
	...
	if (
		["enter", "escape", "tab", "arrowup", "arrowdown", "arrowleft", "arrowright", "backspace", "delete"].includes(
			normalized,
		)
	)
		return normalized;
