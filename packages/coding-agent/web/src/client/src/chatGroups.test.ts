...
describe("groupChatMessages", () => {
		...
		expect(groupChatMessages(messages)).toEqual([
			{
				...
				startIndex: 0,
				endIndex: 0,
				messages: [{ role: "assistant", parts: [{ type: "thinking", text: "hidden" }] }],
			},
		...
		expect(groupChatMessages(messages)).toEqual([
			{
				...
				startIndex: 0,
				endIndex: 0,
				messages: [{ role: "assistant", parts: [{ type: "thinking", text: "plan" }] }],
			},
			{
				kind: "message",
				index: 0,
		...
		expect(groupChatMessages([message])).toEqual([
			{
				...
				startIndex: 0,
				endIndex: 0,
				messages: [{ role: "assistant", parts: [{ type: "thinking", text: "hidden" }], meta: message.meta }],
			},
			{
				kind: "message",
				index: 0,
				message: { role: "assistant", parts: [{ type: "text", text: "shown" }], meta: message.meta },
			},
		]);
	});
	...
	it("keeps a stable group end index when older events are prepended into a group", () => {
		...
		).toMatchObject({ kind: "group", startIndex: 8, endIndex: 9 });
	});
});

describe("summarizeChatGroup", () => {
	it("summarizes special event groups", () => {
		expect(summarizeChatGroup([{ ...text("assistant", "a"), source: "compaction" }])).toBe(
			"1 history compaction summary",
		);
		expect(
			summarizeChatGroup([
	...
	it("summarizes mixed groups by role counts", () => {
		expect(summarizeChatGroup([text("tool", "a"), text("system", "b"), text("tool", "c")])).toBe(
			"3 events · 2 tool · 1 system",
		);
	});
});
