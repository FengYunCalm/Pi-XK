...
import {
	ChatDisclosureController,
	type ChatDisclosureSnapshot,
	type ChatDisclosureStorage,
	parseDisclosureSnapshot,
} from "./chatDisclosure";
...
describe("parseDisclosureSnapshot", () => {
	...
	it("hydrates object storage", () => {
		expect(parseDisclosureSnapshot({ open: ["a"], closedDefaultOpen: ["b", false] })).toEqual({
			open: ["a"],
			closedDefaultOpen: ["b"],
		});
	});
