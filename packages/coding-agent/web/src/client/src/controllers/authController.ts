...
export class AuthController {
	...
	async selectLoginProvider(providerId: string, authType?: AuthType): Promise<void> {
		...
		const provider = dialog.providers.find(
			(candidate) => candidate.id === providerId && (authType === undefined || candidate.authType === authType),
		);
	...
	private updateOAuthFlow(flow: OAuthFlowState): void {
		...
		const previousInput =
			existing?.step === "oauth" && existing.flow.flowId === flow.flowId ? (existing.inputValue ?? "") : "";
		const previousRequestId =
			existing?.step === "oauth" ? (existing.flow.prompt?.requestId ?? existing.flow.select?.requestId) : undefined;
