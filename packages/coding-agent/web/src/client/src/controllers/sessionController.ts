...
import { type GlobalSessionEvent, SessionSocket, type SessionUiEvent } from "../sessionSocket";
import {
...
export class SessionController {
	...
	preferredSession(
		...
		return selectPreferredSession(sessions, {
			targetSessionId,
			latestSessionId: this.sessionSelection.latestSessionId(cwd),
		});
	}
	...
	async selectSession(session: SessionInfo, options?: { updateUrl?: boolean | undefined }) {
		...
		this.setState({
			...
			isLoadingEarlierMessages: false,
			isReceivingPartialStream: false,
			status: this.getState().sessionStatuses[session.id],
			activity: this.getState().sessionActivities[session.id],
		});
		try {
			...
			const [page, status] = await Promise.all([
				this.api.messages(session.id, { limit: MESSAGE_PAGE_SIZE }),
				this.api.status(session.id),
			]);
	...
	async runShell(text: string) {
		...
		try {
			...
			this.setState({
				messages: [...this.getState().messages, textMessage("system", String(error))],
				error: String(error),
			});
		}
	}
	...
	async refreshSelectedSession(sessionId = this.getState().selectedSession?.id): Promise<void> {
		...
		try {
			...
			const [page, status] = await Promise.all([
				this.api.messages(sessionId, { limit: MESSAGE_PAGE_SIZE }),
				this.api.status(sessionId),
			]);
	...
	private async recreateCachedNewSession(
		...
		try {
			...
			this.setState({
				sessions: [
					cachedReplacement,
					...this.getState().sessions.filter((candidate) => candidate.id !== session.id),
				],
				error: "",
			});
	...
	private applyCommandResult(result: CommandResult) {
		...
		if (message !== undefined && message !== "")
			this.setState({
				messages: [
					...
					textMessage(result.type === "unsupported" ? "system" : "tool", message),
				],
			});
		if (result.type === "done" && result.session) {
			...
			const sessions = [
				result.session,
				...this.getState().sessions.filter((session) => session.id !== result.session?.id),
			];
	...
	private applyStatus(status: SessionStatus) {
		...
		const clearsStaleActivity =
			state.sessionActivities[status.sessionId]?.phase === "active" && !isSessionActive(status);
		this.setState({
			...
			...(clearsStaleActivity
				? { sessionActivities: omitSessionActivity(state.sessionActivities, status.sessionId) }
				: {}),
		...
		if (this.catchupStreamSessionId === status.sessionId && !status.isStreaming)
			this.finishStreamCatchup(status.sessionId);
	}
...
function sessionMessageCountPatch(
	...
	const sessionsChanged = state.sessions.some(
		(session) => session.id === sessionId && session.messageCount !== messageCount,
	);
	const sessions = sessionsChanged
		? state.sessions.map((session) => (session.id === sessionId ? { ...session, messageCount } : session))
		: undefined;
	const selectedSession =
		state.selectedSession?.id === sessionId && state.selectedSession.messageCount !== messageCount
