...
export function selectPreferredTerminal(
	terminals: TerminalInfo[],
	options?: { targetTerminalId?: string | undefined; latestTerminalId?: string | undefined },
): TerminalInfo | undefined {
	const targetTerminalId = options?.targetTerminalId;
	if (targetTerminalId !== undefined && targetTerminalId !== "")
		return terminals.find((terminal) => terminal.id === targetTerminalId);

	const latestTerminalId = options?.latestTerminalId;
	if (latestTerminalId !== undefined && latestTerminalId !== "") {
		return (
			terminals.find((terminal) => terminal.id === latestTerminalId) ??
			terminals.find((terminal) => !terminal.exited) ??
			terminals[0]
		);
	}
