...
import {
	InMemoryWorkspaceSelectionMemory,
	selectPreferredWorkspace,
	type WorkspaceSelectionMemory,
...
export class WorkspaceController {
	...
	forgetProject(projectId: string): void {
		...
		const workspacesByProjectId = Object.fromEntries(
			Object.entries(this.getState().workspacesByProjectId).filter(([candidate]) => candidate !== projectId),
		);
	...
	async selectProject(project: Project, target?: RouteTarget) {
		...
		try {
			...
			const workspace = selectPreferredWorkspace(workspaces, {
				...
				latestWorkspaceId: this.workspaceSelection.latestWorkspaceId(project.id),
			});
			if (workspace)
