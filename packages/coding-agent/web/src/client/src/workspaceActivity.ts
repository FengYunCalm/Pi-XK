...
function matchedProjectActivities(
	project: Project,
	knownWorkspaces: Workspace[],
	activities: Record<string, WorkspaceActivity>,
): WorkspaceActivity[] {
	const knownWorkspacePaths = new Set(
		knownWorkspaces.filter((workspace) => workspace.projectId === project.id).map((workspace) => workspace.path),
	);
	...
	for (const activity of Object.values(activities)) {
		if (activity.cwd === project.path || activity.cwd.startsWith(`${project.path}/`))
			matched.set(activity.cwd, activity);
	}
