      ...
      <div class="backdrop" @mousedown=${() => this.onCancel?.()}>
        <section @mousedown=${(event: MouseEvent) => {
          ...
          <div class="options">
            ${
					actions.length === 0
						? html`<div class="empty">No actions found.</div>`
						: actions.map(
								(action, index) => html`
              <button class=${index === this.selectedIndex ? "selected" : ""} ${scrollWhenSelected(index === this.selectedIndex, action.id)} @click=${() => {
						this.run(action);
					}}>
                <span class="main">
	...
	private filteredActions(): AppAction[] {
		...
		return this.actions
			...
			.filter((action) => {
				...
				const haystack = [action.title, action.description ?? "", action.group ?? "", action.shortcut ?? ""]
					.join(" ")
					.toLowerCase();
