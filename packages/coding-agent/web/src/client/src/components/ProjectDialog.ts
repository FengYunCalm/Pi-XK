      ...
      <div class="backdrop" @click=${() => this.onCancel?.()}>
        <section @click=${(event: Event) => {
          ...
          <div class="body">
            <label>
              Project folder
              <input .value=${this.path} @input=${(event: InputEvent) => {
						this.onPathInput(event);
					}} @keydown=${(event: KeyboardEvent) => {
						this.onKeyDown(event);
					}} placeholder="/path/to/project or ~/code/project" autofocus />
            </label>
            <div class="suggestions">
              ${this.loading ? html`<div class="hint">Loading folders…</div>` : null}
              ${this.suggestions.map(
						(suggestion, index) => html`
                <button class=${index === this.selected ? "selected" : ""} @click=${() => {
							this.pick(suggestion);
						}}>
                  ${suggestion.path}
                </button>
            ...
            <label class="check">
              <input type="checkbox" .checked=${this.createMissing} @change=${(event: InputEvent) => {
						this.onCreateMissingChange(event);
					}} />
          ...
          <footer>
            ...
            <button class="primary" ?disabled=${this.path.trim() === ""} @click=${() => {
					this.submit();
				}}>Add project</button>
          </footer>
        </section>
