...
export function actionMenuPanelStyle(target: EventTarget | null, options: ActionMenuPanelStyleOptions = {}): string {
	if (typeof HTMLElement === "undefined" || typeof window === "undefined" || !(target instanceof HTMLElement))
		return "";
	...
	const placement =
		availableBelow < ACTION_MENU_MIN_USEFUL_HEIGHT_PX && availableAbove > availableBelow
			? [
					`bottom: ${px(viewportHeight - trigger.top + ACTION_MENU_GAP_PX)};`,
					`max-height: ${px(Math.max(0, availableAbove))};`,
				]
...
function actionMenuBounds(target: HTMLElement): ActionMenuRect {
	...
	if (typeof ShadowRoot !== "undefined" && root instanceof ShadowRoot && root.host instanceof HTMLElement)
		return root.host.getBoundingClientRect();
