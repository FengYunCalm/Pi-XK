...
import { type ChatGroup, groupChatMessages, summarizeChatGroup } from "../chatGroups";
...
function randomPartialStreamNoticeBody(): string {
	return (
		partialStreamNoticeBodies[Math.floor(Math.random() * partialStreamNoticeBodies.length)] ??
		partialStreamNoticeBodies[0]
	);
}
...
export class ChatView extends LitElement {
	...
	private savePreviousSessionScrollPosition(previousSessionId: unknown): void {
		if (typeof previousSessionId !== "string" || previousSessionId === "" || previousSessionId === this.sessionId)
			return;
	...
	protected override willUpdate(changed: Map<string, unknown>): void {
		...
		if (changed.has("isReceivingPartialStream") || (changed.has("sessionId") && this.isReceivingPartialStream))
			this.syncPartialStreamNoticeBody();
		if (changed.has("messages"))
			this.pinnedToBottom = this.pinnedToBottom && (this.didChatHeightChange() || this.isNearBottom());
	}
	...
	protected override updated(changed: Map<string, unknown>): void {
		...
		if (changed.has("messages") || changed.has("hasMore") || changed.has("loadingMore"))
      ...
      <div class="chat-wrap">
        ...
        <div class="chat" @scroll=${() => {
          ...
          ${repeat(
					groups,
					(group) =>
						group.kind === "message" ? this.messageAnchorKey(group.index) : this.groupRenderKey(group.startIndex),
					(group, index) =>
						group.kind === "message"
	...
	private groupedMessages(): ChatGroup[] {
		if (this.groupedMessagesInput === this.messages && this.groupedMessagesStart === this.messageStart)
	...
	private renderSessionActivity() {
		if (this.isReceivingPartialStream)
			return html`
	...
	private renderMessageGroup(messages: ChatLine[], startIndex: number, endIndex: number, defaultOpen: boolean) {
      ...
      <details class=${defaultOpen ? "msg event-group live" : "msg event-group"} data-index=${startIndex} data-scroll-anchor-id=${this.groupAnchorKey(startIndex)} ?open=${open} @toggle=${(
			event: Event,
		) => {
	...
	private messageMetaLabel(message: ChatLine): { short: string; full: string } {
		...
		const fullParts = [time?.full, model === undefined ? undefined : `Model: ${model}`].filter(
	...
	private renderPart(part: ChatPart, message?: ChatLine) {
		...
		if (part.type === "thinking")
			return html`<details class="part"><summary>thinking</summary><formatted-text .text=${part.text}></formatted-text></details>`;
	...
	private onTouchMove(event: TouchEvent) {
		...
		if (this.touchStartY !== undefined && y !== undefined && y > this.touchStartY && this.canScrollUp())
	...
	restoreScrollPosition() {
		...
		this.restoreScrollFrame = requestAnimationFrame(() => {
			...
			this.withSuppressedScrollSave(() => {
				const result = this.scrollController.restorePosition(sessionId, this.chat, this.scrollAnchorElements(), {
	...
	restorePrependScrollAnchor(anchor: PrependScrollAnchor | undefined): void {
		...
		const settle = () => {
			...
			restorePrependScrollAnchor(
				chat,
				anchor,
				anchor.markerId === undefined ? undefined : this.scrollMarkerAt(anchor.markerId),
			);
			this.lastScrollTop = chat.scrollTop;
			frames += 1;
	...
	private updateConversationRailPosition(): void {
		...
		this.currentConversationIndex = clampNumber(
			this.pinnedToBottom ? this.messageStart + this.messages.length - 1 : this.messageStart,
			0,
			Math.max(0, total - 1),
		);
	}
