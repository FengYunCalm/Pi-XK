...
import { type CompletionItem, promptEditorStyles } from "./shared";
...
export class PromptEditor extends LitElement {
	...
	override render() {
      ...
      <footer class=${shellMode ? "shell-mode" : ""}>
        ...
        <div class="actions">
          ...
          <button ?disabled=${this.disabled} title=${queuesInput ? "Queue until the current activity finishes" : "Send message"} @click=${() => {
					this.send("followUp");
				}}>${queuesInput ? "Queue" : "Send"}</button>
          ${
					this.canSteer && !this.isCompacting
						? html`<button ?disabled=${this.disabled} title="Steer the current response before the next model call" @click=${() => {
								this.send("steer");
							}}>Steer</button>`
						: null
				}
	...
	private renderCompactStatus() {
		...
		const provider =
			status.model?.provider !== undefined && status.model.provider !== "" ? `${status.model.provider}/` : "";
		return html`
			...
			state: EditorState.create({
				...
				extensions: [
					...
					EditorView.contentAttributes.of((view) =>
						inputAssistanceContentAttributes(view.state.sliceDoc(0, view.state.selection.main.head)),
					),
					...
					keymap.of([
						...
						{
							key: "Shift-Enter",
							run: (view) => insertNewlineContinueMarkup(view) || insertNewlineAndIndent(view),
						},
	...
	private async refreshCompletions() {
		...
		if (trigger.kind === "command" && this.sessionId !== undefined && this.sessionId !== "") {
			...
			const files = await api
				.files(this.cwd, trigger.query, { scope: trigger.fileScope })
				.catch(emptyFileSuggestions);
			if (version !== this.requestVersion) return;
			this.completions = files.slice(0, 12).map((file) => {
				const insertText = fileInsertText(
					file.path,
					trigger.quoted === true,
	...
	private currentTrigger():
		| {
				...
				from: number;
				to: number;
				fileScope?: "tracked" | "all" | undefined;
				allPrefix?: "@ " | "!@" | undefined;
				quoted?: boolean;
		  }
		| undefined {
		...
		if (beforeToken.endsWith("@ "))
			return { kind: "file", query: token, from: tokenStart - 2, to: cursor, fileScope: "all", allPrefix: "@ " };
		if (token.startsWith("/") && tokenStart === 0)
			return { kind: "command", query: token.slice(1), from: tokenStart, to: cursor };
		if (token.startsWith("!@"))
			return {
				kind: "file",
				query: token.slice(2),
		...
		if (token.startsWith("@"))
			return { kind: "file", query: token.slice(1), from: tokenStart, to: cursor, fileScope: "tracked" };
		return undefined;
	}

	private currentQuotedTrigger(
		beforeCursor: string,
		cursor: number,
	):
		| {
		...
		if (prefix.endsWith("!@"))
			return {
				kind: "file",
				query: beforeCursor.slice(quoteStart + 1),
	...
	private pick(item: CompletionItem) {
		...
		if (!editor) return;
		const suffix =
			item.kind === "file" && (item.insertText.endsWith("/") || item.cursorOffset !== undefined) ? "" : " ";
		const cursor = item.replaceFrom + (item.cursorOffset ?? item.insertText.length) + suffix.length;
		const replaceTo =
			item.insertText.endsWith('"') && this.draft.slice(item.replaceTo).startsWith('"')
				? item.replaceTo + 1
				: item.replaceTo;
		editor.dispatch({
