...
import { type TerminalCommandRun, type TerminalInfo, terminalSocket, terminalsApi, type Workspace } from "../api";
import { selectFallbackTerminal, selectPreferredTerminal } from "../controllers/terminalSelection";
import {
	createTerminalSoftKeysDefaultEnvironmentMedia,
	hasTerminalSoftKeysPreference,
	initialTerminalSoftKeysEnabled,
	isTerminalSoftKeysDefaultEnvironment,
...
export class TerminalPanel extends LitElement {
	...
	@property({ attribute: false }) onSelectTerminal: (
	...
	private selectPreferredLoadedTerminal(options?: { replaceUrl?: boolean | undefined }): void {
		...
		if (terminal === undefined && this.selectedTerminalId !== undefined)
			terminal = selectFallbackTerminal(this.terminals);
		this.selectTerminalIdInView(terminal?.id);
		if (
			terminal?.id !== this.selectedTerminalId ||
			(terminal === undefined && this.selectedTerminalId !== undefined)
		) {
	...
	private async loadCommandRuns(): Promise<void> {
		...
		if (workspace === undefined) return;
		try {
			const commandRuns = await terminalsApi.listCommandRuns({
				projectId: workspace.projectId,
				workspaceId: workspace.id,
			});
			this.commandRuns = commandRuns;
			this.cancellingRunIds = this.cancellingRunIds.filter((runId) =>
				commandRuns.some((run) => run.id === runId && isCommandRunPending(run)),
			);
			this.updateCommandRunPolling(this.hasPendingCommandRuns(commandRuns));
		} catch (error) {
			this.error = error instanceof Error ? error.message : String(error);
	...
	private updateCommandRunPolling(shouldPoll: boolean): void {
		if (shouldPoll && this.commandRunPollTimer === undefined) {
			this.commandRunPollTimer = window.setInterval(() => {
		...
		try {
			...
			if (message.type === "exit") {
				...
				this.terminals = this.terminals.map((item) =>
					item.id === terminalId
						? { ...item, exited: true, ...(message.exitCode === undefined ? {} : { exitCode: message.exitCode }) }
						: item,
				);
				void this.loadCommandRuns();
			}
	...
	private renderCommandRunNotice() {
        ...
        <section class="command-run-notice running">
          ...
          <button class="danger" ?disabled=${cancelling} @click=${() => {
					void this.cancelCommandRun(run);
				}}>${cancelling ? "Cancel sent…" : "Cancel command"}</button>
        </section>
      `;
		}
        ...
        <section class=${`command-run-notice ${run.status}`}>
          ...
          <button ?disabled=${continuing} @click=${() => {
					void this.continueTerminal(terminal.id);
				}}>${continuing ? "Starting shell…" : "Continue in shell"}</button>
        </section>
      `;
		}
		return null;
	...
	private renderSoftKeys() {
      ...
      <terminal-soft-keys
        ...
        .onInput=${(data: string, options: TerminalSoftKeyInputOptions) => {
...
function commandRunCompletionLabel(run: TerminalCommandRun): string {
	if (run.status === "succeeded")
		return `Command succeeded${run.exitCode === undefined ? "" : ` with exit code ${String(run.exitCode)}`}`;
...
function parseServerMessage(data: string): ServerTerminalMessage {
	...
	if (record["type"] === "output" && typeof record["data"] === "string")
		return {
			type: "output",
			data: record["data"],
			...(typeof record["replay"] === "boolean" ? { replay: record["replay"] } : {}),
		};
	if (record["type"] === "exit")
		return { type: "exit", ...(typeof record["exitCode"] === "number" ? { exitCode: record["exitCode"] } : {}) };
