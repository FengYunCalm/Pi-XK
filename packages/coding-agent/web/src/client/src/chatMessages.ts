...
export function appendText(messages: ChatLine[], role: ChatLine["role"], text: string): ChatLine[] {
	...
	if (last?.role === role)
		return [...messages.slice(0, -1), { ...last, parts: [...last.parts, { type: "text", text }] }];
...
export function appendThinking(messages: ChatLine[], text: string): ChatLine[] {
	...
	if (last?.role === "assistant")
		return [...messages.slice(0, -1), { ...last, parts: [...last.parts, { type: "thinking", text }] }];
...
export function normalizeMessage(message: unknown): ChatLine[] {
	...
	if (getString(message, "role") === "bashExecution")
		return [withMessageMeta(normalizeBashExecution(message), message)];
	...
	const displayRole =
		role === "assistant" && visible.length > 0 && visible.every((part) => part.type === "skillRead") ? "skill" : role;
	return visible.length > 0
		? [withMessageMeta({ role: displayRole, parts: visible, ...(source === undefined ? {} : { source }) }, message)]
		: [];
}
...
function normalizeSkillInvocation(parts: ChatPart[]): ChatLine[] | undefined {
	...
	return [
		{
			role: "user",
			parts: [{ type: "skillInvocation", name: skill.name, location: skill.location, content: skill.content }],
		},
		...(skill.userMessage === undefined
			? []
			: [{ role: "user" as const, parts: [{ type: "text" as const, text: skill.userMessage }] }]),
	];
}
...
function normalizeBashExecution(message: unknown): ChatLine {
	...
	const lines =
		getBoolean(message, "excludeFromContext") === true
			? ["excluded from context", "", `$ ${command}`]
			: [`$ ${command}`];
...
function normalizeContent(content: unknown, message: unknown): ChatPart[] {
	...
	return content
		...
		.map((part) =>
			part.type === "text" && getString(message, "role") === "toolResult"
...
function mergeToolResultInto(
	lines: ChatLine[],
	target: { lineIndex: number; partIndex: number },
	...
	lines[target.lineIndex] = {
		...line,
		parts: [...line.parts.slice(0, target.partIndex), next, ...line.parts.slice(target.partIndex + 1)],
	};
	return true;
}
