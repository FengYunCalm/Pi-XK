...
describe("chat message normalization", () => {
	it("normalizes simple text messages and drops empty content", () => {
		...
		).toEqual([textMessage("user", "hello"), textMessage("system", "system text")]);
	});
	...
	it("normalizes tool calls and tool results", () => {
		expect(
			...
			{
				role: "assistant",
				parts: [{ type: "toolCall", toolName: "bash", summary: "npm test", args: { command: "npm test" } }],
			},
		]);
		expect(
			normalizeMessage({
	...
	it("extracts skill invocation blocks into dedicated skill and user messages", () => {
		expect(
			normalizeMessage({
				role: "user",
				content:
					'<skill name="playwright" location="/skills/playwright">\nUse browser\n</skill>\n\nNow test the UI',
			}),
		).toEqual([
			{
				role: "user",
				parts: [
					{ type: "skillInvocation", name: "playwright", location: "/skills/playwright", content: "Use browser" },
				],
			},
	...
	it("normalizes skill reads into skill chat lines", () => {
		expect(
			normalizeMessage({
				role: "assistant",
				content: [
					{ type: "toolCall", name: "read", arguments: { path: "/home/user/.agents/skills/playwright/SKILL.md" } },
				],
			}),
		).toEqual([
			{
...
describe("appendText", () => {
	...
	it("adds a text part to the previous same-role non-text message", () => {
		expect(
			appendText([{ role: "assistant", parts: [{ type: "thinking", text: "plan" }] }], "assistant", "answer"),
		).toEqual([
			{
				role: "assistant",
				parts: [
